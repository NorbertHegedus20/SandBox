...
export default class Content {
    public content(req: http.IncomingMessage, res: http.ServerResponse): void {
        ...
        let vaneilyen: boolean = true;
        res.write(`${vaneilyen}`);

        let orszagok: Array<string>;
        orszagok = ['Magyarország','Németország','Franciaország'];
        for (let index = 0; index < orszagok.length; index++) {
                res.write(orszagok[index])  ;          
        }

        var szemely: [number,string] = [15,"Béla"];
        szemely.push(2,"Károly");
        res.write(szemely);
